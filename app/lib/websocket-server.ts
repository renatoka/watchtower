  ...
  io.on('connection', (socket) => {
    ...
    socket.on('subscribe', (endpointId) => {
      if (endpointId) {
        ...
        socket.join('global');
        console.log(
          `📡 Client ${socket.id} subscribed to global updates`
        );
      }
    });
    ...
    socket.on('unsubscribe', (endpointId) => {
      if (endpointId) {
        ...
        socket.leave('global');
        console.log(
          `📡 Client ${socket.id} unsubscribed from global updates`
        );
      }
    });
    ...
    socket.on('requestFullUpdate', async () => {
      try {
        ...
        const allStatistics =
          await monitoringEngine.getAllUptimeStatuses();
...
export function broadcastUptimeUpdate(statistics: UptimeStatistics) {
  if (io) {
    ...
    io.to(`endpoint:${statistics.endpointId}`).emit(
      'uptimeUpdate',
      statistics
    );
  }
